		...
		getMainData() {
			...
			const callback = (res) => {
				if (res.info.data.length > 0) {
					...
					self.data.addYf = parseFloat(parseFloat(self.data.skuData.price)+parseFloat(self.data.skuData.postage)).toFixed(2)
				} else {
			...
			const callback = (user, res) => {
				...
				if (self.data.skuData.id != '' && self.data.skuData.id != undefined) {
					...
					if(self.data.transport==-1){
						...
						api.pathTo('/pages/order_get/order_get?group_no=' + self.data.scene + '&&user_no=' + self.data.user_no,
							'nav')
					}
