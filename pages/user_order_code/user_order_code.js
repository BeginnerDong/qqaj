...
Page({
	...
	data: {
		...
		isLoadAll: false,
		isQQ: false,
		searchType: ['商品名称', '微信昵称', '收货人姓名', '收货人电话'],
		isChoose: false,
		index: -1,
		getBefore:{}
	},
	...
	chooseSearchType(e) {
		const self = this;
		console.log(e);
		self.data.sForm.keywords = '';
		self.data.index = e.detail.value;
		self.data.isChoose = true
		self.data.getBefore = {};
		delete self.data.searchItem.snap_address;
		delete self.data.searchItem.snap_product_list;
		self.setData({
			searchType: self.data.searchType,
	...
	changeLi(e) {
		const self = this;
		if (self.data.noClick) {
			return
		};
		if (self.data.liCurr != e.target.dataset.value) {
			self.data.noClick = true;
			self.data.mainData = [];
			self.setData({
				web_mainData: self.data.mainData
			})
	...
	searchTwo() {
		const self = this;
		if (self.data.sForm.keywords!='') {
			if(self.data.clock){
				return
			};
			self.data.clock = true;
			if(self.data.searchType[self.data.index]=='商品名称'){
				self.data.searchItem.snap_product_list = ['LIKE',['%'+self.data.sForm.keywords+'%']]
			}else if(self.data.searchType[self.data.index]=='微信昵称'){
				self.data.getBefore = {
					user: {
						tableName: 'user',
						middleKey: 'user_no',
						key: 'user_no',
						searchItem: {
							nickname:['LIKE',['%'+self.data.sForm.keywords+'%']]
						},
						condition: 'in'
					}
				}
			}else if(self.data.searchType[self.data.index]=='收货人姓名'){
				self.data.searchItem.snap_address = ['LIKE',['%'+self.data.sForm.keywords+'%']]
			}else if(self.data.searchType[self.data.index]=='收货人电话'){
				self.data.searchItem.snap_address = ['LIKE',['%'+self.data.sForm.keywords+'%']]
			};
			self.getMainData(true)
		} else if (self.data.sForm.passage2 == '') {
	...
	getMainData(isNew) {
		...
		const callback = (res) => {
			...
			self.data.noClick = false;
			self.data.clock = false;
			self.setData({
				web_mainData: self.data.mainData
			})
	...
	copyPhone(e) {
		...
		wx.setClipboardData({
			data: self.data.mainData[index].snap_address.phone
		})
	},
	...
	copyAddress(e){
		...
		var index = e.target.dataset.index;
		wx.setClipboardData({
			data: self.data.mainData[index].snap_address.city+self.data.mainData[index].snap_address.detail
		})
	},
