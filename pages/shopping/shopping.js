...
Page({
	...
	pay() {
		...
		for (var i = 0; i < self.data.mainData.length; i++) {
			if (self.data.mainData[i].isSelect) {
				skuDatas.push({
					...
					transport:self.data.mainData[i].transport
				});
			}
		};
		var status = false;
		for (var i = 0; i < skuDatas.length; i++) {
			if(i<skuDatas.length-1){
				if(parseInt(skuDatas[i].transport)!=parseInt(skuDatas[i+1].transport)){
					status = true
		...
		if (status) {
			wx.showModal({
				title:'提示',
				content:'请将【到店自提】商品与【快递邮寄】商品分别下单',
				showCancel:false,
			})
			return
		};
		...
		for (var i = 0; i < skuDatas.length; i++) {
			if(skuDatas[i].transport==1){
				api.pathTo('/pages/order_delivery/order_delivery',
					'nav')
			}else if(skuDatas[i].transport==2){
				api.pathTo('/pages/order_get/order_get',
					'nav')
			}
		}
